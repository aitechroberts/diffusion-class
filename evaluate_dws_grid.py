"""
DWS-RX-DDIM KID Evaluation — 2D Grid (tau x n_rx_steps)

Full grid sweep: for every (tau, n_rx_steps) pair, generates 5000 samples
using DWS-RX-DDIM, computes KID, and writes a single CSV with all results.

NFE = 1 (distilled warm-start) + n_rx_steps.

Usage:
    python evaluate_dws_grid.py \\
        --base_checkpoint /data/.../ddpm_final.pt \\
        --distilled_checkpoint /data/.../pd_stage5_final.pt \\
        --tau_values 0.05,0.1,0.2,0.3,0.4 \\
...
def main():
    parser = argparse.ArgumentParser(
        description="DWS-RX-DDIM KID grid sweep (tau x n_rx_steps)")
    ...
    parser.add_argument("--rx_step_counts", type=str,
    ...
    parser.add_argument("--output_csv", type=str,
                        default="benchmark/kid_dws_grid.csv")
    ...
    total_configs = len(tau_values) * len(step_counts)
    print(f"DWS-RX-DDIM grid: {len(tau_values)} tau x {len(step_counts)} steps "
          f"= {total_configs} configs, {args.num_samples} samples each")
    ...
    for n_rx in step_counts:
        for tau in tau_values:
            ...
            output_dir = os.path.join(
                ...
                f"dws_grid_rx{n_rx}_tau{tau:.2f}",
            )
            ...
            print(f"[{config_num}/{total_configs}] "
                  f"n_rx={n_rx}  tau={tau:.2f}  NFE={nfe}")
            ...
            with torch.no_grad():
                ...
                while remaining > 0:
                    ...
                    save_individual_images(
                        samples, output_dir, start_idx=sample_idx)
                    sample_idx += bs
                    remaining -= bs
                    pbar.update(bs)
                pbar.close()
            ...
            rows.append((
                "dws_rx_ddim", tau, n_rx, nfe, kid_mean, kid_std,
            ))
    ...
    print(f"\n{'='*60}")
    print(f"Grid sweep complete — {len(rows)} configs")
    print(f"Results: {args.output_csv}")
    print(f"{'='*60}")

    print(f"\n{'n_rx':>5s} {'tau':>6s} {'NFE':>4s} {'KID':>10s}")
    print("-" * 30)
    for row in rows:
        print(f"{row[2]:>5d} {row[1]:>6.2f} {row[3]:>4d} {row[4]:>10.6f}")
